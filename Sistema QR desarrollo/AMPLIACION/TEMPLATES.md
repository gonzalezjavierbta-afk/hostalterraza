...
- **Carpeta de silos:** cada template_id tiene su carpeta por categoría: `css/templates/fiesta/` (f9, f10, f11), `css/templates/` por categoría (cine, campana, hosteleria…).
...
**ADRs citados:** ADR-041, ADR-042, ADR-043, ADR-031. **Errores históricos:** §5, §8, §12.
...
3. **Clase de ámbito — DOBLE NOTACIÓN DE CASING:** el kernel escribe en `document.body` DOS clases con casing distinto (≈L692-694): `tpl-{template_id}` (tal cual viene en `eventos.template_id` — p.ej. `tpl-F10`) y `tpl-{id}` (en minúsculas — p.ej. `tpl-f10`). **Por qué existe:** algunos eventos traen `template_id` en mayúsculas ("F10") y otros en minúsculas ("f10"); el JS usa `document.body.className.includes('tpl-f8')` / `.indexOf('tpl-f9')` con minúsculas para activar sus subconmutaciones, y el CSS debe cubrir ambos casos. **Regla:** todo selector de silo debe declarar la doble forma `.tpl-fXX, .Tpl-FXX` (o `.Tpl-FXX, .tpl-fXX`) para que el silo aplique sin importar el casing que traiga la base de datos.
...
**ADRs citados:** ADR-005, ADR-008, ADR-006, ADR-010. **Reglas citadas:** Mandato 4 (Escudo GOLD), Mandato 13 (armonía HSL).
