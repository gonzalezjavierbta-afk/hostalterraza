...
- **Leer en lugar de asumir**: usa los nombres de campo REALES que guarda admin.html (`t.link_reserva`/`t.descripcion`, nunca `t.link`/`t.desc`) — ver BUG-012. Usa la nomenclatura obligatoria de Sistema QR desarrollo/BLUEPRINT.md sección 3 (`ciudad`, `descripcion`, `telefono`, `precio_desde`, `creado_en`).
...
1. Verifica el ARCHIVO REAL (ADR-006): nunca asumas lo que dice Sistema QR desarrollo/TASKS.md/Sistema QR desarrollo/NEXT.md sobre el archivo — el historial demuestra que los docs se quedan atrás.
